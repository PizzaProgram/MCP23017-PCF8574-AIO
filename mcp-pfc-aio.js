	...
	function mcp_pcf_inNode(_inputConfig) {
		...
		this.on('input', function(msg, send, done) { // triggering an Immediate read if payload is True or 1 >> to support Interrupts
			if (msg.payload) {
				...
				msg.immediateReadSuccess 	= _parentChipNode.myTimer(true); // result is True, if succesfully red, false if any error occured during read
				msg.readingTimeLengthMs		= _parentChipNode.readLength;
				msg.AllStatesRaw			= _parentChipNode.AllStates;
				msg.AllBinary				= msg.immediateReadSuccess  ? _parentChipNode.AllStates.toString(2) : "";
	...
	function mcp_pcf_outNode(_OutputConfig) {
		...
		this.on('input', function(msg) {
			if (!node.initOK) {
				...
				node.initOK = _parentChipNode.initializeBit(node.bitNum, false, false, this.id);
				if (!node.initOK) {return null}
			}

			if ( msg.payload == "All0" ) {
				if (log2consol) console.log("  MCP/PCF > Set ALL pins to 0000...  Chip Addr=" + _parentChipNode.addr);
				node.setOutput(-1, false, node);
			} else
			if ( msg.payload == "All1" ) {
				if (log2consol) console.log("  MCP/PCF > Set ALL pins to 1111...  Chip Addr=" + _parentChipNode.addr);
			...
			} else
			{
				...
				if (node.setOutput(node.bitNum, _invPinOn, node)) {
					showState(node, _invPinOn, _parentChipNode.globalState);
